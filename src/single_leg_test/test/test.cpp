...
int main (int argc, char* argv[]) {
    ...
    body_a = Body (3.927, Vector3d (0., 0., 0.05), Matrix3d(0.0057, 0. , 0., 0., 0.0057, 0., 0., 0., 0.0049 ));//Body (const double &mass, const Math::Vector3d &com, const Math::Matrix3d &inertia_C)
    joint_a = Joint(
        JointTypeRevolute,
        Vector3d (0., 0., 1.)
    );//body_a's mass 1.0, center of mass and the inertia at the center of mass/or the radius gradius.

    Matrix3d B0toB1Rotation = rotx(-PI/2);

    body_a_id = model->AddBody(0, SpatialTransform(B0toB1Rotation,Vector3d(0., 0., 0.)), joint_a, body_a);
    body_b = Body (3.927, Vector3d (0.05, 0., 0.), Matrix3d (0.0049, 0., 0., 0., 0.0057, 0., 0., 0., 0.0057));
        joint_b = Joint (
        JointTypeRevolute,
        Vector3d (0., 0., 1.)
    );
    //pay attention to the transform order of the matrix!!!!!!from the right multiply.
    Matrix3d B1toB2Rotation = rotx(-PI/2)*rotz(PI/2);
    body_b_id = model->AddBody(body_a_id, SpatialTransform(B1toB2Rotation,Vector3d(0., 0., 0.1)), joint_b, body_b);

    body_c = Body (3.927, Vector3d (0.05, 0., 0.),Matrix3d (0.0049, 0., 0., 0., 0.0057, 0., 0., 0., 0.0057));
        joint_c = Joint (
        JointTypeRevolute,
        Vector3d (0., 0., 1.)
    );

    body_c_id = model->AddBody(body_b_id, Xtrans(Vector3d(0.1, 0., 0.)), joint_c, body_c);
    ...
    unsigned int length_of_data = 10000;
